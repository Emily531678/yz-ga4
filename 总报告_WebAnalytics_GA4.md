...
时间范围（Funnel Exploration）：本月（Apr 1–Apr 29, 2026）

- Step 1 `vote_page_view`：29（100%）
- Step 2 `vote_option_select`：25（Step1→Step2 转化率：86.21%，流失：13.79%）
- Step 3 `vote_submit`：22（Step2→Step3 转化率：88.00%，流失：12.00%）

**问题定位：**

- 两处都有流失：Step1→Step2 表示部分用户进入页面但没有开始选择；Step2→Step3 表示部分用户已选择但未完成提交
- 设备分群（Device category）显示不同设备的阻力点不同：
  - desktop 的 Step1→Step2 流失更高（25%），说明桌面端“开始选择”的引导仍可加强
  - mobile 的 Step2→Step3 流失更高（15.79%），说明移动端“提交完成”仍有优化空间

### 3.4 优化方案（Fix a problem）

针对 Step1→Step2 与 Step2→Step3 的流失，提出以下可落地优化：
...
2. 降低“开始选择”的心理成本
   ...
   - 优化桌面端首屏与视觉引导：确保投票区醒目可见，减少 desktop 用户“只浏览不选择”的概率

3. 提升移动端提交完成率
   - 增强按钮可见性/点击面积，减少打断（将 alert 改为页面内提示）
   - 在选择后提示“已选择 X，请点击按钮完成投票”
...
时间范围（Events 报表）：最近 28 天（Apr 1–Apr 28, 2026）

- 总用户数：34
- `vote_submit`（整体投票提交）：83 次（18 位用户）
- `vote_submit_custom`（自定义事件，筛选 `place = 瘦西湖`）：58 次（16 位用户）
- `vote_option_select`：125 次（25 位用户）
- `vote_page_view`：89 次（29 位用户）
- `vote_results_view`：175 次（18 位用户）
